...
if (isset($_POST['saveSupplierIngredient'])) {
    ...
    // Check if ingredient_id and price arrays are set
    if (isset($_POST['ingredient_id']) && isset($_POST['price'])) {
        $ingredient_ids = $_POST['ingredient_id'];
        $prices = $_POST['price'];

        // Loop through the ingredient IDs and prices to insert them into the database
        for ($i = 0; $i < count($ingredient_ids); $i++) {
            $ingredient_id = $ingredient_ids[$i];
            $price = $prices[$i];

            // Prepare SQL statement to prevent SQL injection
            $stmt = $conn->prepare("INSERT INTO supplier_ingredients (supplier_id, ingredient_id, price) VALUES (?, ?, ?)");
            $stmt->bind_param("iid", $supplier_id, $ingredient_id, $price);
    ...
    if (isset($_POST['ingredient_id']) && isset($_POST['price'])) {
        ...
        try {
            for ($i = 0; $i < count($ingredient_ids); $i++) {
                ...
                if (!empty($ingredient_id) && !empty($price)) {
                    $stmt = $conn->prepare("
                        INSERT INTO supplier_ingredients (supplier_id, ingredient_id, price) 
                        VALUES (?, ?, ?) 
