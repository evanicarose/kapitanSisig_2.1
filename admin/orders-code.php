...
if (isset($_POST['productIncDec'])) {
    ...
    if (mysqli_num_rows($checkProduct) > 0) {
        ...
        foreach ($_SESSION['productItems'] as $key => $item) {
            if ($item['product_id'] == $productId) {
                // Stop increment if quantity equals or exceeds stock
                if ($quantity >= $row['quantity'] + 1) {
                    $quantity = $row['quantity']; // Set to max available
                    $flag1 = true; // Max quantity reached
                } else {
                    // Update the session with new quantity
    ...
    if ($flag1) {
        jsonResponse(500, 'success', "Maximum quantity reached! Only " . $row['quantity'] . " items available.");
    } else if ($flag) {
...
if (isset($_POST['proceedToPlaceBtn'])) {
    ...
    // Loop through the product items in the session to ensure the latest quantities
    foreach ($_SESSION['productItems'] as $product) {
        $productId = $product['product_id'];
        $quantity = $product['quantity']; // Make sure this quantity is the correct one

        // Add logic to process the order with the correct quantity
        // For example, inserting into the `order_items` table
    }

    // Continue with customer check and placing the order
