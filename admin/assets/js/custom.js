    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            if (res.status == 200) {
                ...
                $('#productArea').load(' #productContent', function() {
                    // Once content is loaded, re-enable the increment button
                    var $incrementBtn = $('.qtyBox').find('.prodId[value="' + prodId + '"]').closest('.qtyBox').find('.prod-increment');
                    $incrementBtn.prop('disabled', false);
                });
                alertify.success(res.message);
            } else if (res.status == 500 && res.message === 'reload') {
                // Reload product area since max quantity is reached
                ...
                var $incrementBtn = $('.qtyBox').find('.prodId[value="' + prodId + '"]').closest('.qtyBox').find('.prod-increment');
                $incrementBtn.prop('disabled', true); // Disable increment button when error occurs
            }
